...
static bool print_digit(const uint8_t *digit, uint8_t virt_row, uint8_t virt_col, uint8_t offset_rows, uint8_t offset_cols) {
...
static void print_display(uint16_t time) {
  ...
  for (uint16_t i = 0; i < (OLED_COLS * OLED_ROWS); i++) {
    ...
    if (!print_digit(first_digit_ptr, virt_row, virt_col, ALL_DIGITS_OFFSET_ROWS, ALL_DIGITS_OFFSET_COLS) &&
	!print_digit(second_digit_ptr, virt_row, virt_col, ALL_DIGITS_OFFSET_ROWS, ALL_DIGITS_OFFSET_COLS + DIGIT_COLS + SPACE_BETWEEN_DIGITS) &&
	!print_digit(third_digit_ptr, virt_row, virt_col, ALL_DIGITS_OFFSET_ROWS, ALL_DIGITS_OFFSET_COLS + 2 * (DIGIT_COLS + SPACE_BETWEEN_DIGITS))) {
      write_data(0x00);
    }
  }
}
